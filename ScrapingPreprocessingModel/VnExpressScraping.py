...
def download_articles_to_csv(article_num: int, href: str, topic: str) -> None:
...
#     download_articles_to_csv(NO_ARTICLES_PER_TOPIC, h, topics[ih])

# Chia thành 6 threads tải cùng lúc
downloadThreads = []
for ih, h in enumerate(href_topics):
    downloadThread = threading.Thread(target=download_articles_to_csv, args=(NO_ARTICLES_PER_TOPIC, h, topics[ih]))
