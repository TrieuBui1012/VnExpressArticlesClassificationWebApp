...
df.to_csv('./Data/VnExpressArticlesPreprocessing.csv', encoding='utf-8', index=False)
...
# Giảm đặc trưng dữ liệu
svd = TruncatedSVD(n_components=300)
matrix_reduced = svd.fit_transform(matrix)
print('Kích thước của dữ liệu sau giảm số đặc trưng:')
print(matrix_reduced.shape)
print('Dữ liệu sau khi giảm số đặc trưng:')
print(matrix_reduced)

# Lưu lại dữ liệu, vectorizer và TruncatedSVD để sử dụng sau này
joblib.dump(vectorizer, 'tfidf_vectorizer.joblib')
joblib.dump(svd, 'truncatedsvd_model.joblib')
print('Lưu TF-IDF Vectorizer và TruncatedSVD thành công!')
df_tfidf = pd.DataFrame(matrix_reduced)
df_tfidf['topic'] = df['topic']
joblib.dump(df_tfidf, './Data/df_tfidf.joblib')
print('Lưu dữ liệu sau khi vector hóa và giảm số đặc trưng thành công!')
